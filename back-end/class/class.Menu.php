...
class Menu extends DB_connect {
	...
	public function __construct($lang) {
		...
		$this->lang = $lang;
		//Запит до бази даних
		$result = $this->db->query("SELECT `cat_id`, `cat_name_{$this->lang}`, `cat_parent`, `cat_class` FROM `categories`");
		if (!$result) {
			return NULL;
		}
		$arr_cat = array();
	...
	public function print_categories($parent = 0) {
		...
		foreach ($this->categories[$parent] as $item) {
			...
			$cat_name = $this->lang == "en" ? $item["cat_name_{$this->lang}"] : $item["cat_name"];

			echo "<li><a href=\"index.php?category={$item["cat_id"]}\"". (!empty($item["cat_class"]) ? " class=\"{$item["cat_class"]}\"" : NULL) .">{$item["cat_name_{$this->lang}"]}</a>";
			if (isset($child)) {
	...
	public function select_categorie($parent = 0, $default_cat) {
		...
		foreach ($this->categories[$parent] as $item) {
			...
			echo "<li><input type=\"radio\" value=\"{$item["cat_id"]}\" id=\"{$item["cat_id"]}\" name=\"post_cat\" form=\"form_post\" ". ($default_cat == $item["cat_id"] ? "checked" : NULL) ."><label for=\"{$item["cat_id"]}\">{$item["cat_name_{$this->lang}"]}</label>";
			if (isset($child)) {
	...
	public function manage_categories($parent = 0) {
		foreach ($this->categories[$parent] as $item) {
			...
			echo "<tr". ($parent != 0 ? " class=\"child\"" : NULL) .">
					<td><span class=\"cat_name\">{$item["cat_name_{$this->lang}"]}</span>
	...
	public function parent_categories() {
		...
		for ($i = 0; $i < count($this->categories[0]); $i++) {
			echo "<option value=\"{$this->categories[0][$i]["cat_id"]}\">{$this->categories[0][$i]["cat_name_{$this->lang}"]}</option>";
		}
		echo "</select>";
	}
}
